...
function helix2_pitch!(pos, conn, nodeID, length, diameter, pitch, nbWires, rWireSection, braidingPattern, wireGap, wireID, clockwise)
    
    nbTotalCells = length * pitch
...
function compute_bs_geom_given_pitch(nbWires, radius, rWireSection, wireGap, length, pitch, braidingPattern)
    ...
    for n in 1:nbWires
        helix2_pitch!(positions, connectivity, nodeID, length, radius*2, pitch, nbWires, rWireSection, braidingPattern, wireGap, n, true)

    end 
    
    for n in 1:nbWires
        helix2_pitch!(positions, connectivity, nodeID, length, radius*2, pitch, nbWires, rWireSection, braidingPattern, wireGap, n, false)
    end
...
function read_vtk_centerline_radius(filename)
...
function get_init_pos_deploy_middle(filename_cl, input, initial_positions_stent, output_dir_crimping)
    ...
    length_stent = η_stent[end] - η_stent[1] + 0.5

    cl = read_vtk_centerline(filename_cl)
    η = vcat(0, cumsum(norm.(diff(cl))))
    length_cl = η[end]-η[1]

    deploy_pos = input*(length_cl-length_stent)
...
function write_vtk_general_structured_mesh(filename, s, dx, x, y, z) 
    ...
    for n in 1:ncells
        write(fid, string(round(s_vec[n],digits=3)))
        write(fid," ")
    end 
